...
Bmi088_error_e Bmi088_init(void) {
    ...
    error |= Verify_acc_chip_id();
    error |= Verify_gyro_chip_id();
    if (1) {  // 将来改成变量控制自检
        error |= Verify_acc_self_test();
    }
    if (1) {  // 将来改成变量控制自检
        error |= Verify_gyro_self_test();
    }
    return error;
}
...
void Bmi088_conf_init(void) {
    // 加速度计初始化
    // 先软重启，清空所有寄存器
    Write_data_to_acc(ACC_SOFTRESET_ADDR, ACC_SOFTRESET_VAL);
    HAL_Delay(50);
    // 打开加速度计电源
    Write_data_to_acc(ACC_PWR_CTRL_ADDR, ACC_PWR_CTRL_ON);
    // 加速度计变成正常模式
    Write_data_to_acc(ACC_PWR_CONF_ADDR, ACC_PWR_CONF_ACT);
    ...
    // 加速度计配置写入
    // 写入范围，+-3g的测量范围
    Write_data_to_acc(ACC_RANGE_ADDR, ACC_RANGE_3G);
    // 写入配置，正常带宽，1600hz输出频率
    Write_data_to_acc(ACC_CONF_ADDR,
...
Bmi088_error_e Verify_acc_chip_id(void) {
    uint8_t chip_id;
    Read_single_data_from_acc(ACC_CHIP_ID_ADDR, &chip_id);
...
Bmi088_error_e Verify_gyro_chip_id(void) {
    uint8_t chip_id;
    Read_single_data_from_gyro(GYRO_CHIP_ID_ADDR, &chip_id);
...
Bmi088_error_e Verify_acc_self_test(void) {
    ...
    Write_data_to_acc(ACC_RANGE_ADDR, ACC_RANGE_24G);
    Write_data_to_acc(ACC_CONF_ADDR, 0xA7);
    HAL_Delay(10);
    Write_data_to_acc(ACC_SELF_TEST_ADDR, ACC_SELF_TEST_POS);
    HAL_Delay(100);
    Read_acc_data(&pos_data);
    Write_data_to_acc(ACC_SELF_TEST_ADDR, ACC_SELF_TEST_NEG);
    HAL_Delay(100);
    Read_acc_data(&neg_data);
    Write_data_to_acc(ACC_SELF_TEST_ADDR, ACC_SELF_TEST_OFF);
    HAL_Delay(100);
...
Bmi088_error_e Verify_gyro_self_test(void) {
    Write_data_to_gyro(GYRO_SELF_TEST_ADDR, GYRO_SELF_TEST_ON);
    uint8_t bist_rdy = 0x00, bist_fail;
    while (bist_rdy == 0) {
        Read_single_data_from_gyro(GYRO_SELF_TEST_ADDR, &bist_rdy);
        bist_rdy = (bist_rdy & 0x02) >> 1;
    }
    Read_single_data_from_gyro(GYRO_SELF_TEST_ADDR, &bist_fail);
...
void Read_acc_data(Acc_raw_data_t *data) {
    ...
    int16_t acc[3];
    Read_single_data_from_acc(ACC_RANGE_ADDR, &range);
    Read_multi_data_from_acc(ACC_X_LSB_ADDR, ACC_XYZ_LEN, buf);
...
void Read_gyro_data(Gyro_raw_data_t *data) {
    ...
    int16_t gyro[3];
    float unit;
    Read_single_data_from_gyro(GYRO_RANGE_ADDR, &range);
    switch (range) {
        case 0x00:
            unit = 16.384;
            break;
...
void Read_acc_sensor_time(float *time) {
    ...
    Read_multi_data_from_acc(SENSORTIME_0_ADDR, SENSORTIME_LEN, buf);
...
void Read_acc_temperature(float *temp) {
    ...
    Read_multi_data_from_acc(TEMP_MSB_ADDR, TEMP_LEN, buf);
