...
=======
Bmi088_instance_t *imu = Bmi088_register(&imu_config);

IT非阻塞模式下BMI088读取流程
数据准备完成标志位:uint8_t BMI088_READY_FLAG 总共8个位 也可以用位域可读性更高

当accel int中断发生,开启DMA SPI传输,完成后将acc ready置位

当gyro int中断发生,开启DMA SPI传输,完成后将gyro ready置位

当温度数据中断(温度传感器在accel内部,也是accel int)发生,开启DMA传输,完成后将温度标志位置位
...
bit 0 :1 bit 1-7: reg address

dummy read,加速度计此时返回的数据无效
...
2. 读回的数据
=======
bit 0: 0 bit1-7: reg address

要写入寄存器的数据(注意没有dummy byte)

注意,陀螺仪和加速度计的读取不同

陀螺仪gyro读取read:

bit 0 :1 bit1-7: reg address
