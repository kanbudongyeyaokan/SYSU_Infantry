...
static void HWT606_Start_Read(void) {
    ...
    // 缓冲区：Gyro(6) + Mag(6) + Angle(6) = 18 字节
    ...
    if (HAL_I2C_Mem_Read(hwt606_dev.hi2c, hwt606_dev.dev_addr, REG_READ_START, 
                         I2C_MEMADD_SIZE_8BIT, buf, READ_LEN, 50) == HAL_OK) 
    {
        // --- 解析角速度 (前6字节) ---
        hwt606_dev.raw.gyro[1] = (int16_t)(buf[0] | (buf[1] << 8)); // 原本是X，现在给Y
        hwt606_dev.raw.gyro[0] = (int16_t)(buf[2] | (buf[3] << 8)); // 原本是Y，现在给X
        hwt606_dev.raw.gyro[2] = (int16_t)(buf[4] | (buf[5] << 8)); // Z轴不变

        // --- 解析角度 (后6字节) ---
        hwt606_dev.raw.angle[1] = (int16_t)(buf[12] | (buf[13] << 8)); // 原本是Roll(0x3D)，现在赋值给 Pitch
        hwt606_dev.raw.angle[0] = (int16_t)(buf[14] | (buf[15] << 8)); // 原本是Pitch(0x3E)，现在赋值给 Roll
...
static bool HWT606_Wait_Data(void) {
    return hwt606_dev.read_success;
}
