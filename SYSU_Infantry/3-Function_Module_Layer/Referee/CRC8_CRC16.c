...
/* CRC16 初始值（多项式 0x8408，即 CRC-CCITT 反转） */
...
/* 验证末字节 CRC8：计算前 (dw_length-1) 字节，与末字节比较，相等返回 1 */
uint32_t verify_CRC8_check_sum(unsigned char *pch_message, unsigned int dw_length)
{
    if ((pch_message == 0) || (dw_length <= 2)) return 0;
    unsigned char ucExpected = get_CRC8_check_sum(pch_message, dw_length - 1, CRC8_INIT);
...
{
    if ((pch_message == 0) || (dw_length <= 2)) return;
    pch_message[dw_length - 1] = get_CRC8_check_sum(pch_message, dw_length - 1, CRC8_INIT);
}

/* 逐字节查表计算 CRC16，返回最终校验值 */
uint16_t get_CRC16_check_sum(uint8_t *pch_message, uint32_t dw_length, uint16_t wCRC)
{
    if (pch_message == NULL) return 0xFFFF;
    uint8_t chData;
    while (dw_length--) {
        chData = *pch_message++;
