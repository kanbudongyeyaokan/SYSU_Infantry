...
void Chassis_task_init(void) {
  ...
  chassis_feedback_pub =
      Pub_register("chassis_feedback", sizeof(Chassis_feedback_info_t));
}
...
void Chassis_init() {
  ...
  chassis_params.wheel_perimeter =
      chassis_params.wheel_radius * 2 * M_PI; // 轮子周长
  ...
  Djimotor_init_config_t cfg[4] = {
                                    ...
                                    .speed_pid =
                                        {
                                            ...
                                            .optimization =
                                                ...
                                                PID_OUTPUT_LIMIT |
                                                PID_DIFFERENTIAL_GO_FIRST |
                                                PID_FEEDFOWARD,
                                            .max_out = 15000,
                                        }},
          .can_init = {.can_handle = &hcan1,
                       .can_id = 0x200,
                       .tx_id = 1,
                       .rx_id = 0x201}
          // .can_init = {.can_handle = &hcan1, .can_id = 0x1FF, .tx_id = 2,
          // .rx_id = 0x206}
      },
            ...
            .speed_pid =
                {
                    ...
                    .LPF_coefficient = 0.8,
                    .feedfoward_coefficient = 0.2,
                    .optimization = PID_TRAPEZOID_INTERGRAL | PID_OUTPUT_LIMIT |
                                    PID_DIFFERENTIAL_GO_FIRST | PID_FEEDFOWARD,
                    .max_out = 15000,
                }},
       .can_init =
           {.can_handle = &hcan1, .can_id = 0x200, .tx_id = 2, .rx_id = 0x202}},
      {.motor_name = "CHASSIS_BL",
       ...
       .can_init =
           {.can_handle = &hcan1, .can_id = 0x200, .tx_id = 3, .rx_id = 0x203}},
      {.motor_name = "CHASSIS_BR",
       ...
       .can_init = {
           .can_handle = &hcan1, .can_id = 0x200, .tx_id = 4, .rx_id = 0x204}}};
 ...
 * @param output 解算输出结果
 */
static void Chassis_omni_kinematics(const Chassis_cmd_send_t *cmd,
                                    Chassis_output_t *output) {
  // 目前以电池所在位置为后方，其对面为正前方
  //  计算各轮子线速度 (rad/s)
  float wheel_linear_speed[4];
  wheel_linear_speed[0] =
      -cmd->vx - cmd->vy -
      cmd->wz *
          (chassis_params.half_wheel_base + chassis_params.half_track_width) *
          MATH_DEG2RAD; // 右前轮
  wheel_linear_speed[1] =
      -cmd->vx + cmd->vy -
      cmd->wz *
          (chassis_params.half_wheel_base + chassis_params.half_track_width) *
          MATH_DEG2RAD; // 左后轮
  wheel_linear_speed[2] =
      cmd->vx + cmd->vy -
      cmd->wz *
          (chassis_params.half_wheel_base + chassis_params.half_track_width) *
          MATH_DEG2RAD; // 左前轮
  wheel_linear_speed[3] =
      cmd->vx - cmd->vy -
      cmd->wz *
          (chassis_params.half_wheel_base + chassis_params.half_track_width) *
          MATH_DEG2RAD; // 右后轮
 ...
 * @param output 解算输出结果
 */
static void Chassis_mecanum_kinematics(const Chassis_cmd_send_t *cmd,
                                       Chassis_output_t *output) {
  // 麦克纳姆轮布局（从俯视图看）：
  // 左前(0)  右前(1)
  // 左后(2)  右后(3)
  ...
  float rotate_compensation =
      cmd->wz * (L + W) / (2.0f * chassis_params.wheel_radius);
  ...
  for (int i = 0; i < 4; i++) {
    float angular_velocity =
        wheel_linear_speed[i] / chassis_params.wheel_radius;           // rad/s
 ...
 * @param output 解算输出结果
 */
void Chassis_kinematics_solve(const Chassis_cmd_send_t *cmd,
                              Chassis_output_t *output) {
  // 根据底盘类型调用对应的解算函数
